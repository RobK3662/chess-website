...
# print(f"{chess_player_info_list}") #Leider gibt es hier noch eine leere Liste aus --> Muss noch so gefixt werden, dass sie die top 50 Spieler als einzelne Listen
...
json_data_leaderboard = get_data_from_webpage(url, headers)
players = json_data_leaderboard['player_id'][0]['draw_count']

# Hier kannst du die Spielerdaten verarbeiten
for player in players:
    player_id = player['player_id']
    username = player['username']
    rating = player['score']
    print(f'Player ID: {player_id}, Username: {username}, Rating: {rating}')